...
export const handler: MessageHandler = (bot) => {
    bot.on("successful_payment", async (ctx) => {
        ...
        await ctx.replyWithDocument(pdf_link, {
            caption: `Grazie per l'acquisto! Ecco il file PDF contenente gli appunti di ${invoice_payload.course}. Per segnalare eventuali errori non esitare a contattare @sAlb98 su Telegram.`
        });
    })
}